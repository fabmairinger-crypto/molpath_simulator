/* MolPath Simulator v2.6.0-rc1 — Home / Help / FOM screen
...
const FOM=Object.freeze({
  version:(window.MolPathVersion&&window.MolPathVersion.fom)||'0.6',
  pdf:(window.MolPathVersion&&window.MolPathVersion.fomPdf)||'docs/fom/MolPath_Flight_Operations_Manual_v0.6.pdf',
  pageBase:(window.MolPathVersion&&window.MolPathVersion.fomPageBase)||'docs/fom/pages',
  pages:Number((window.MolPathVersion&&window.MolPathVersion.fomPages)||50)
});
...
#${ROOT_ID} .mph-manual-card{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:18px;align-items:center;border:1px solid rgba(15,118,110,.24);border-radius:20px;background:linear-gradient(135deg,rgba(232,248,246,.96),rgba(255,255,255,.92));padding:18px 20px;box-shadow:0 10px 28px rgba(15,35,55,.055)}#${ROOT_ID} .mph-manual-icon{display:grid;place-items:center;width:58px;height:58px;border-radius:16px;background:#0f766e;color:#fff;font-size:.85rem;font-weight:950;letter-spacing:.08em;box-shadow:0 10px 22px rgba(15,118,110,.2)}#${ROOT_ID} .mph-manual-copy{min-width:0}#${ROOT_ID} .mph-manual-kicker{color:#0f766e;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px}#${ROOT_ID} .mph-manual-copy strong{display:block;color:#0f4c75;font-size:1.08rem;margin-bottom:3px}#${ROOT_ID} .mph-manual-copy span{display:block;color:#667085;font-size:.84rem;line-height:1.45}#${ROOT_ID} .mph-manual-actions{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}#${ROOT_ID} .mph-manual-actions .mph-btn{white-space:nowrap}
#${ROOT_ID} .mph-manual-layer{position:fixed;inset:0;z-index:8;display:none;background:#eaf3f8;color:#0b1b34}#${ROOT_ID} .mph-manual-layer.open{display:flex;flex-direction:column}#${ROOT_ID} .mph-manual-head{flex:0 0 auto;display:flex;align-items:center;gap:10px;min-height:64px;padding:9px 14px;background:rgba(255,255,255,.97);border-bottom:1px solid #d5e2ec;box-shadow:0 5px 18px rgba(15,35,55,.07);backdrop-filter:blur(10px)}#${ROOT_ID} .mph-manual-head-title{min-width:0;flex:1}#${ROOT_ID} .mph-manual-head-title b{display:block;color:#0f4c75;font-size:.98rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#${ROOT_ID} .mph-manual-head-title span{display:block;color:#667085;font-size:.72rem;margin-top:1px}#${ROOT_ID} .mph-manual-tools{display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end}#${ROOT_ID} .mph-manual-tool{display:grid;place-items:center;min-width:38px;height:38px;padding:0 10px;border:1px solid #d2e0ea;border-radius:11px;background:#fff;color:#0f4c75;font-weight:900;cursor:pointer}#${ROOT_ID} .mph-manual-tool:disabled{opacity:.4;cursor:not-allowed}#${ROOT_ID} .mph-manual-page-label{min-width:92px;text-align:center;color:#526276;font-size:.78rem;font-weight:850}#${ROOT_ID} .mph-manual-body{flex:1;min-height:0;overflow:auto;padding:20px;display:flex;justify-content:center;align-items:flex-start}#${ROOT_ID} .mph-manual-page{display:block;width:min(100%,980px);height:auto;background:#fff;box-shadow:0 18px 55px rgba(15,35,55,.17);border:1px solid #d5e2ec}#${ROOT_ID} .mph-manual-download{display:inline-flex;align-items:center;justify-content:center;text-decoration:none}
@media(max-width:900px){#${ROOT_ID} .mph-manual-card{grid-template-columns:auto 1fr}#${ROOT_ID} .mph-manual-actions{grid-column:1/-1;justify-content:flex-start;padding-left:76px}#${ROOT_ID} .mph-hero{grid-template-columns:1fr;padding:26px}#${ROOT_ID} .mph-logo-zone{order:-1;min-height:180px}#${ROOT_ID} .mph-logo{width:min(48%,190px)}#${ROOT_ID} .mph-grid{grid-template-columns:repeat(2,minmax(0,1fr))}#${ROOT_ID} .mph-tutorial{grid-template-columns:1fr}#${ROOT_ID} .mph-mock{min-height:240px}#${ROOT_ID} .mph-mode-grid{grid-template-columns:1fr}}
@media(max-width:560px){#${ROOT_ID} .mph-manual-card{grid-template-columns:46px 1fr;gap:11px;padding:14px}#${ROOT_ID} .mph-manual-icon{width:46px;height:46px;border-radius:13px;font-size:.72rem}#${ROOT_ID} .mph-manual-actions{grid-column:1/-1;padding-left:0;display:grid;grid-template-columns:1fr 1fr;width:100%}#${ROOT_ID} .mph-manual-actions .mph-btn{width:100%;padding-left:10px;padding-right:10px}#${ROOT_ID} .mph-manual-head{align-items:flex-start;flex-wrap:wrap;padding:8px}#${ROOT_ID} .mph-manual-head-title{flex:1 1 calc(100% - 50px)}#${ROOT_ID} .mph-manual-tools{width:100%;justify-content:space-between}#${ROOT_ID} .mph-manual-page-label{min-width:auto;flex:1}#${ROOT_ID} .mph-manual-body{padding:8px}#mp21HomeBtn{min-width:36px!important;width:36px!important;padding:0!important}#${ROOT_ID} .mph-shell{width:min(100% - 18px,1180px);padding:10px 0 18px;gap:10px}#${ROOT_ID} .mph-top{padding:0 3px}#${ROOT_ID} .mph-mini-brand span{display:none}#${ROOT_ID} .mph-lang label{display:none}#${ROOT_ID} .mph-lang select{max-width:132px}#${ROOT_ID} .mph-hero{border-radius:19px;padding:19px 17px;gap:12px}#${ROOT_ID} .mph-logo-zone{min-height:132px}#${ROOT_ID} .mph-logo{width:min(42%,142px)}#${ROOT_ID} h1{font-size:clamp(2.2rem,14vw,3.5rem)!important;margin-bottom:12px!important}#${ROOT_ID} .mph-sub{font-size:.94rem;margin-bottom:17px}#${ROOT_ID} .mph-actions{display:grid;grid-template-columns:1fr}#${ROOT_ID} .mph-btn{width:100%}#${ROOT_ID} .mph-stats{margin-top:15px}#${ROOT_ID} .mph-grid{grid-template-columns:1fr;gap:9px}#${ROOT_ID} .mph-card{min-height:0;padding:14px;display:grid;grid-template-columns:42px 1fr;column-gap:11px}#${ROOT_ID} .mph-card-icon{grid-row:1/3;margin:0;width:40px;height:40px}#${ROOT_ID} .mph-card strong{margin:1px 0 3px}#${ROOT_ID} .mph-card span{grid-column:2}#${ROOT_ID} .mph-dialog-layer{padding:0;align-items:stretch}#${ROOT_ID} .mph-dialog{max-height:100dvh;border-radius:0;border:0;width:100%}#${ROOT_ID} .mph-dialog-body{padding:15px}#${ROOT_ID} .mph-mock{grid-template-columns:62px 1fr;min-height:220px;padding:9px}#${ROOT_ID} .mph-foot{padding:5px 4px 0}}
...
<div class="mph-shell">
  ...
  <section class="mph-manual-card" aria-label="${esc(t.manualTitle||baseCopy().manualTitle||'Flight Operations Manual')}">
    <div class="mph-manual-icon">FOM</div>
    <div class="mph-manual-copy"><div class="mph-manual-kicker">${esc(t.manualKicker||baseCopy().manualKicker||'Complete operational reference')} · v${esc(FOM.version)}</div><strong>${esc(t.manualTitle||baseCopy().manualTitle||'Flight Operations Manual')}</strong><span>${esc(t.manualText||baseCopy().manualText||'Complete cockpit handbook and operational reference.')}</span></div>
    <div class="mph-manual-actions"><button class="mph-btn primary" type="button" data-action="manual-open">${esc(t.manualOpen||baseCopy().manualOpen||'Open manual')}</button><button class="mph-btn" type="button" data-action="manual-download">${esc(t.manualDownload||baseCopy().manualDownload||'Download PDF')}</button></div>
  </section>
...
<div class="mph-manual-layer" aria-hidden="true" role="dialog" aria-modal="true" aria-label="${esc(t.manualTitle||baseCopy().manualTitle||'Flight Operations Manual')}"><header class="mph-manual-head"><div class="mph-manual-head-title"><b>${esc(t.manualTitle||baseCopy().manualTitle||'Flight Operations Manual')}</b><span>v${esc(FOM.version)} · ${esc(VERSION)}</span></div><div class="mph-manual-tools"><button class="mph-manual-tool" type="button" data-action="manual-prev" title="${esc(t.manualPrevious||baseCopy().manualPrevious||'Previous')}">‹</button><span class="mph-manual-page-label" id="mphManualPageLabel"></span><button class="mph-manual-tool" type="button" data-action="manual-next" title="${esc(t.manualNext||baseCopy().manualNext||'Next')}">›</button><button class="mph-manual-tool" type="button" data-action="manual-zoom-out" title="${esc(t.manualZoomOut||baseCopy().manualZoomOut||'Zoom out')}">−</button><button class="mph-manual-tool" type="button" data-action="manual-zoom-in" title="${esc(t.manualZoomIn||baseCopy().manualZoomIn||'Zoom in')}">+</button><a class="mph-manual-tool mph-manual-download" href="${esc(FOM.pdf)}" download="MolPath_Flight_Operations_Manual_v${esc(FOM.version)}.pdf" title="${esc(t.manualDownload||baseCopy().manualDownload||'Download PDF')}">⇩</a><button class="mph-manual-tool" type="button" data-action="manual-close" title="${esc(t.manualClose||baseCopy().manualClose||'Close manual')}">×</button></div></header><div class="mph-manual-body" id="mphManualBody"><img class="mph-manual-page" id="mphManualPage" alt="${esc(t.manualTitle||baseCopy().manualTitle||'Flight Operations Manual')}"></div></div>`;}
...
function bind(){
  ...
  root.addEventListener('click',e=>{const a=e.target.closest('[data-action]');if(!a)return;const action=a.dataset.action;if(action==='start')hide();else if(action==='library'){hide();setTimeout(openLibrary,0);}else if(action==='how')openHow();else if(action==='tutorial')openTutorial(0);else if(action==='modes')openModes();else if(action==='news')openNews();else if(action==='manual-open')openManual();else if(action==='manual-download')downloadManual();else if(action==='manual-close')closeManual();else if(action==='manual-prev')setManualPage(manualPage-1);else if(action==='manual-next')setManualPage(manualPage+1);else if(action==='manual-zoom-in')setManualZoom(manualZoom+.25);else if(action==='manual-zoom-out')setManualZoom(manualZoom-.25);else if(action==='close-dialog')closeDialog();else if(action==='tut-next')openTutorial(Number(a.dataset.step||0)+1);else if(action==='tut-back')openTutorial(Number(a.dataset.step||0)-1);else if(action==='tut-done')closeDialog();});}
  const sel=root.querySelector('#mpHomeLang');if(sel){sel.addEventListener('change',e=>{try{if(window.MolPathI18n)window.MolPathI18n.setLang(e.target.value);else localStorage.setItem('molpath_lang',e.target.value);}catch(_){}setTimeout(refresh,20);});try{if(window.MolPathEnhanceLanguageSelect)window.MolPathEnhanceLanguageSelect(sel);}catch(_){}}
...
function hide(){if(!root)return;closeManual();entered=true;root.hidden=true;document.body.classList.remove('mp-home-open');try{if(lastFocus&&lastFocus.focus)lastFocus.focus({preventScroll:true});}catch(_){} }
...
function manualPad(n){return String(n).padStart(2,'0');}
function manualLayer(){return root&&root.querySelector('.mph-manual-layer');}
function updateManualControls(){if(!root)return;const t=tx();const layer=manualLayer();if(!layer)return;const label=root.querySelector('#mphManualPageLabel');if(label)label.textContent=(t.manualPage||baseCopy().manualPage||'Page')+' '+manualPage+' '+(t.manualOf||baseCopy().manualOf||'of')+' '+FOM.pages;const prev=layer.querySelector('[data-action="manual-prev"]'),next=layer.querySelector('[data-action="manual-next"]');if(prev)prev.disabled=manualPage<=1;if(next)next.disabled=manualPage>=FOM.pages;const img=root.querySelector('#mphManualPage');if(img){img.style.width=manualZoom===1?'min(100%,980px)':Math.round(980*manualZoom)+'px';img.style.maxWidth=manualZoom===1?'100%':'none';}}
function renderManualPage(){if(!root)return;const img=root.querySelector('#mphManualPage');if(!img)return;img.src=FOM.pageBase+'/page-'+manualPad(manualPage)+'.webp';img.alt=(tx().manualTitle||baseCopy().manualTitle||'Flight Operations Manual')+' - '+manualPage+'/'+FOM.pages;updateManualControls();const body=root.querySelector('#mphManualBody');if(body)body.scrollTo({top:0,left:0,behavior:'auto'});[manualPage-1,manualPage+1].filter(n=>n>=1&&n<=FOM.pages).forEach(n=>{const pre=new Image();pre.src=FOM.pageBase+'/page-'+manualPad(n)+'.webp';});}
function setManualPage(n){manualPage=Math.max(1,Math.min(FOM.pages,Number(n)||1));renderManualPage();}
function setManualZoom(z){manualZoom=Math.max(.75,Math.min(2,Math.round(Number(z)*4)/4));updateManualControls();}
function openManual(){create();closeDialog();const layer=manualLayer();if(!layer)return;lastFocus=document.activeElement;layer.classList.add('open');layer.setAttribute('aria-hidden','false');document.body.classList.add('mp-home-manual-open');renderManualPage();const close=layer.querySelector('[data-action="manual-close"]');try{close&&close.focus({preventScroll:true});}catch(_){}}
function closeManual(){const layer=manualLayer();if(!layer)return;layer.classList.remove('open');layer.setAttribute('aria-hidden','true');document.body.classList.remove('mp-home-manual-open');try{lastFocus&&lastFocus.focus&&lastFocus.focus({preventScroll:true});}catch(_){}}
function downloadManual(){const a=document.createElement('a');a.href=FOM.pdf;a.download='MolPath_Flight_Operations_Manual_v'+FOM.version+'.pdf';a.style.display='none';document.body.appendChild(a);a.click();setTimeout(()=>{try{a.remove();}catch(_){}},0);}
...
document.addEventListener('keydown',e=>{if(!root||root.hidden)return;const ml=manualLayer();if(ml&&ml.classList.contains('open')){if(e.key==='Escape'){e.preventDefault();closeManual();return;}if(e.key==='ArrowRight'||e.key==='PageDown'){e.preventDefault();setManualPage(manualPage+1);return;}if(e.key==='ArrowLeft'||e.key==='PageUp'){e.preventDefault();setManualPage(manualPage-1);return;}}if(e.key==='Escape'&&root.querySelector('.mph-dialog-layer.open')){e.preventDefault();closeDialog();}});
...
window.MolPathHomeScreen=Object.freeze({version:VERSION,show,hide,refresh,openTutorial:()=>{show();openTutorial(0);},openManual:()=>{show();openManual();}});
})();
